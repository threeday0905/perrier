...
function XConfig(options) {
    ...
    function parseConfig(confData, confPath) {
        ...
        return parser.replaceAllMatchProps(confData, function(prop) {
            ...
            return isString(prop) && (prop.lastIndexOf(confFlag, 0) === 0);
        }, function(prop) {
        ...
        lookupRelatedConfProp(configHoster, function(prop, propKey, propPath) {
            ...
            if (samePathValue) {
                ...
                if (parser.isObject(samePathValue)) {
                    ...
                    console.warn('[xconf] ' +
                        'config flag: obj.%s (%s) has been overwritten by "%s"',
                        propPath, prop.originalDesc, samePathValue
                    );
            ...
            value: function() {
                ...
                items.forEach(function(item, index) {
                    ...
                    try {
                        ...
                        mergeToHoster(self, confData);

                        debug(null, confInfo.name, index);
                    } catch (ex) {
                        debug(ex);
                    }
                });
