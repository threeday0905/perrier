...
function XConfig(options) {
    var confFlag, rootPath, globalField;
     ...
     */
    function lookupRelatedConfProp(configHoster, callback) {
     ...
     */
    function mergeToHoster(configHoster, configItem) {
        expect.all(configHoster, configItem).isObject();
        ...
        lookupRelatedConfProp(configHoster, function(prop, propKey, propPath) {
            ...
            if (samePathValue) {
                ...
                if (_.isObject(samePathValue)) {
                    ...
                    if (!prop.otherFields._missFields) {
                        prop.otherFields._missFields = [];
                    }
                    prop.otherFields._missFields.push(samePathValue);
                }
            }
            return prop;
        });
    ...
    function rebuildHoster(configHoster) {
        ...
        lookupRelatedConfProp(configHoster, function(prop) {
            ...
            var realtedPath = path.resolve(prop.parentPath, prop.filePath),
                relatedConf = confLoader.load(realtedPath);

            // error handling: if config is not exists
            if (!relatedConf) {
                relatedConf = {
                    _fileNotFound: realtedPath
                };
            }
            ...
            _.assign(relatedConf, prop.otherFields);
        ...
        load: {
            value: function(/* source, ... [, debug ]*/ ) {
                ...
                _.each(items, function(item, index) {
                    try {
                        var info   = loadConfig(item),
                            config = parseConfig(info.data, info.path);

                        mergeToHoster(self, config);

                        debug(null, info.name, index);
                    } catch (ex) {
                        debug(ex);
                    }
                });
...
XConfig.create = function createXConfig(options) {
    return new XConfig(options);
};
